...
- Module registry: `apps/erp/src/modules/index.ts` (`moduleRegistry`). A module is one entry there: a manifest with the backend `key`, a `navArea`, its nav entry and its `React.lazy` routes. The sidebar and the `/<module>/*` routes are built from the registry filtered by `me.enabled_modules` (`<RequireModule>`); the installer is Settings → Modules over `/api/v1/modules/`.
