...
- Module kit: `examples/pos` — a Vite IIFE build with the runtime as externals (`@kaabe/runtime` typed by `examples/pos/src/runtime-types.d.ts`). `npm run typecheck:module` and `npm run build:module` produce `examples/pos/dist/module.js` (+ `module.css`), which `kaabe-backend` packs with the backend app (`manage.py pack_module`). A bundle must import nothing but the runtime: React comes from the host, never bundled.
