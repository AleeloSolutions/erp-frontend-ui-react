...
| DocumentCustomizer        | Composite | In progress (pre-Approved) | UI    | Migrated from `modules/sales`; layouts serve invoices, quotations, contracts |
| DocumentCustomizerModal   | Composite | In progress (pre-Approved) | UI    | Dialog shell; app owns persistence via `onSave`                              |
| DocumentPreview           | Composite | In progress (pre-Approved) | UI    | 3 layouts x 3 table styles, settings-driven                                  |
| InvoicePrintDocument*     | Composite | In progress (pre-Approved) | UI    | Fixed reproductions of an external report theme — not themeable              |
