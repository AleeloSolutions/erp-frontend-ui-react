...
export function listItems(params: ListParams = {}): Promise<Page<Item>> {
  return api.apiGetPage<Item>(`/v1/inventory/items/?${query(params)}`);
}

export function getItem(uuid: string) {
  return api.apiGet<Item>(`/v1/inventory/items/${uuid}/`);
}
...
export function updateItem(uuid: string, input: ItemInput) {
  return api.apiPatch<Item>(`/v1/inventory/items/${uuid}/`, input);
}

export function deleteItem(uuid: string) {
  return api.apiDelete<void>(`/v1/inventory/items/${uuid}/`);
}
